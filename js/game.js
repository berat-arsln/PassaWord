const HARFLER = [
  ...
  /* ========================================================================= */
      ...
      async function sorulariYukle() {
        document.getElementById("yuklemeMesaj").textContent =
          "Sorular yükleniyor...";
      ...
      function daireOlustur() {
        ...
        HARFLER.forEach((harf, i) => {
          ...
          konteyner.insertBefore(
            top,
            konteyner.querySelector(".merkez-gosterge")
          );
        });
      }
      ...
      function ekraniYenile() {
        if (
          !document.getElementById("oyunEkrani").classList.contains("gizli")
        ) {
          daireOlustur();
  ...
  const ekler = ["lar","ler","lık", "lik", "luk", 
                "lük", "lı", "li", "lu", "lü", "mak",
                 "mek", "ma", "me","k","i","ca","cı","ci","cu","çı"];
...
gecmiseKaydet({
  ...
  tarih: new Date().toISOString(),
    ...
    `;
        ...
        const favoriMi = sonOyun && (profil?.favoriler || []).some(f => f.tarih === sonOyun.tarih);
