  ...
  sonucBandi.classList.remove('gizli');
      ...
      function adminOneriYukle(filtre = "soru") {
        ...
        [
          ...
          font-size:13px;
          background:${
            filtre === f ? renk : "rgba(255,255,255,0.08)"
          };color:${filtre === f ? "#fff" : "#8899bb"};`;
          ...
          onValue(
            ...
            (anlık) => {
              ...
              const girdiler = Object.entries(veri).filter(([_, v]) => {
                if (filtre === "soru")
                  return v.altTur && v.altTur.includes("Soru");
                if (filtre === "sikayet")
              ...
              girdiler
                ...
                .forEach(([key, oneri]) => {
                  const soruOneriMi =
                    oneri.altTur && oneri.altTur.includes("Soru");
      ...
      window.adminOneriReddet = function (key, btn) {
        ...
        remove(ref(veritabani, `geribildirimler/${key}`))
          .then(() => {
            toastGoster("✅ Silindi");
            document
              .querySelector(`.admin-oneri-kart[data-key="${key}"]`)
              ?.remove();
          })
...
`;
  ...
  window.pwSekmeGec = function(sekme, btn) {
    ['skor','oyuncu','ayarlar'].forEach(s => {
      document.getElementById('pwPanel_' + s).style.display = s === sekme ? 'block' : 'none';
      const b = document.getElementById('pwSekmeBtn_' + s);
      if (b) {
        b.style.background = s === sekme ? 'rgba(255,255,255,0.11)' : 'transparent';
        b.style.color = s === sekme ? '#fff' : 'var(--metin-soluk)';
      }
    });
    if (sekme === 'skor') pwSkorListesiYukle();
    if (sekme === 'ayarlar') { pwBakimDurumKontrol(); pwDuyuruListesiYukle(); }
  };

        /* Sıralamayı yükle */
        setTimeout(() => pwSekmeGec('ayarlar', document.getElementById('pwSekmeBtn_ayarlar')), 0);
...
window.pwSiralamayiGor = function() {
  ...
  modal.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(5px);z-index:200;display:flex;align-items:flex-end;justify-content:center;';
  modal.innerHTML = `
...
<div style="font-size:18px;font-weight:800;text-align:center;margin-bottom:16px;color:#fff;">🏆 SIRALAMA (TOP 50)</div>
  ...
  const sorgu = query(ref(veritabani, 'skorlar'), orderByChild('puan'), limitToLast(50));
  get(sorgu).then(anlık => {
    const veri = anlık.val();
    const liste = document.getElementById('pwSiralamaListe');
    if (!veri) { liste.innerHTML = `<div style="text-align:center;color:var(--metin-soluk);padding:24px;">Kayıt yok.</div>`; return; }
    const dizisi = Object.entries(veri).map(([k,v]) => ({_key:k,...v})).sort((a,b) => b.puan - a.puan);
    liste.innerHTML = '';
    dizisi.forEach((s, i) => {
      const tarih = s.tarih ? new Date(s.tarih).toLocaleDateString('tr-TR') : '—';
      const satir = document.createElement('div');
      satir.style.cssText = 'display:flex;align-items:center;gap:8px;padding:9px 10px;border-radius:10px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);margin-bottom:6px;';
      satir.innerHTML = `
        <div style="min-width:24px;font-size:13px;font-weight:800;color:${i<3?'#ffd600':'var(--metin-soluk)'};">${i+1}</div>
        <div style="flex:1;">
          <div style="font-size:14px;font-weight:700;color:#fff;">${s.isim||'?'}</div>
          <div style="font-size:11px;color:var(--metin-soluk);margin-top:1px;">${tarih} &nbsp;<span style="color:#00e676;">✓${s.dogru||0}</span> <span style="color:#ff1744;">✗${s.yanlis||0}</span> <span style="color:#ffd600;">~${s.pas||0}</span></div>
        </div>
        <div style="font-size:15px;font-weight:800;color:#4fc3f7;">${s.puan}</div>
        <button onclick="pwSkorSil('${s._key}',this)" style="background:rgba(255,23,68,0.15);border:1px solid rgba(255,23,68,0.3);border-radius:6px;color:#ff6b6b;font-size:11px;font-weight:700;padding:4px 8px;cursor:pointer;">🗑</button>
      `;
      ...
      function pwSkorListesiYukle() {
        ...
        get(sorgu).then(anlık => {
          ...
          dizisi.forEach((s, i) => {
            const tarih = s.tarih ? new Date(s.tarih).toLocaleDateString('tr-TR') : '—';
            const satir = document.createElement('div');
            satir.style.cssText = 'display:flex;align-items:center;gap:8px;padding:7px 8px;border-radius:8px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);margin-bottom:5px;';
            satir.innerHTML = `
              <div style="min-width:22px;font-size:12px;font-weight:800;color:${i<3?'#ffd600':'var(--metin-soluk)'};">${i+1}</div>
              <div style="flex:1;min-width:0;">
                <div style="font-size:13px;font-weight:700;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${s.isim || '?'}</div>
              ...
              <div style="font-size:14px;font-weight:800;color:#4fc3f7;margin-right:4px;">${s.puan}</div>
              <button onclick="pwSkorSil('${s._key}',this)" style="background:rgba(255,23,68,0.15);border:1px solid rgba(255,23,68,0.3);border-radius:6px;color:#ff6b6b;font-size:11px;font-weight:700;padding:4px 8px;cursor:pointer;white-space:nowrap;">🗑</button>
            `;
            konteyner.appendChild(satir);
          });
        }).catch(() => {
          if (konteyner) konteyner.innerHTML = `<div style="color:#ff6b6b;font-size:13px;">Yüklenemedi.</div>`;
        });
      }
      ...
      window.pwSkorSil = function(key, btn) {
        ...
        remove(ref(veritabani, `skorlar/${key}`))
          .then(() => { btn.closest('div[style]').remove(); toastGoster('✅ Skor silindi'); })
          .catch(() => { toastGoster('❌ Silinemedi!'); btn.textContent = '🗑'; btn.disabled = false; });
      };
      ...
      window.pwSkorEkle = function() {
        ...
        if (!isim) { toastGoster('⚠️ İsim boş olamaz!'); return; }
        if (puan < 0) { toastGoster('⚠️ Puan 0 veya üzeri olmalı!'); return; }

        push(ref(veritabani, 'skorlar'), {
          isim, puan, dogru, yanlis, pas,
          tarih: new Date().toISOString()
        }).then(() => {
          toastGoster('✅ Skor eklendi!');
          ['pwEkleIsim','pwEklePuan','pwEkleDogru','pwEkleYanlis','pwEklePas']
      ...
      window.pwSkorTablosuSifirlaOnay = function() {
        if (!confirm('TÜM genel skor tablosu silinecek! Bu geri alınamaz. Devam edilsin mi?')) return;
        if (!confirm('Son kez onaylıyor musun? Tüm skorlar kalıcı silinecek.')) return;
        remove(ref(veritabani, 'skorlar'))
          .then(() => { toastGoster('✅ Skor tablosu sıfırlandı'); pwSkorListesiYukle(); })
      ...
      window.pwOyuncuAra = async function() {
        const kod = document.getElementById('pwOyuncuKodAra')?.value.trim().toUpperCase();
      ...
      function pwOyuncuKartHtml(kod, p) {
        ...
        const enYuksek = skorSayisi > 0 ? Math.max(...(p.skorlar||[]).map(s=>s.puan)) : 0;
        return `
          <div style="background:rgba(255,255,255,0.04);border:1px solid rgba(206,147,216,0.2);border-radius:10px;padding:10px 12px;">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">
              <div>
                <div style="font-size:14px;font-weight:800;color:#fff;">${p.ad || '?'}</div>
      ...
      window.pwOyuncuSil = function(kod, btn) {
        ...
        remove(ref(veritabani, `yedekler/${kod}`))
          ...
          .catch(() => { toastGoster('❌ Silinemedi!'); btn.textContent = '🗑 Sil'; btn.disabled = false; });
      };
...
window.pwDuyuruEkle = async function() {
  ...
  if (!metin) { toastGoster('Duyuru metni girin!'); return; }
  ...
  try {
    ...
    pwDuyuruListesiYukle();
  } catch(e) { toastGoster('❌ Eklenemedi!'); }
};
...
window.pwDuyuruSil = async function(key) {
  try {
    ...
    pwDuyuruListesiYukle();
  } catch(e) { toastGoster('❌ Silinemedi!'); }
};
  ...
  get(ref(veritabani, 'ayarlar/duyurular')).then(snap => {
    ...
    Object.entries(veri)
      ...
      .forEach(([key, d]) => {
        ...
        if (d.sureDakika > 0) {
          ...
          else if (d.sureDakika < 1440) sureYazi = `${Math.round(d.sureDakika/60)} saat`;
          else sureYazi = `${Math.round(d.sureDakika/1440)} gün`;
        }
        const div = document.createElement('div');
        div.style.cssText = 'background:rgba(255,255,255,0.04);border:1px solid rgba(255,214,0,0.2);border-radius:8px;padding:9px 12px;margin-bottom:6px;display:flex;align-items:flex-start;gap:8px;';
        div.innerHTML = `
          <div style="flex:1;min-width:0;">
            <div style="font-size:13px;font-weight:700;color:#ffd600;word-break:break-word;">${d.metin}</div>
    ...
    if (konteyner) konteyner.innerHTML = `<div style="color:#ff6b6b;font-size:13px;">Yüklenemedi.</div>`;
      ...
      function _pwBakimUiGuncelle(aktif) {
        ...
        if (durum) durum.textContent = aktif ? '🔴 Aktif — Oyunculara giriş kapalı' : '🟢 Kapalı — Oyun açık';
        if (btn) {
          btn.textContent = aktif ? '🟢 Kapat' : '🔴 Aç';
          btn.style.background = aktif ? 'rgba(0,200,83,0.15)' : 'rgba(255,23,68,0.15)';
          btn.style.borderColor = aktif ? 'rgba(0,200,83,0.35)' : 'rgba(255,23,68,0.35)';
...
`;
      ...
      function adminSoruKartRender(kart, soru, index) {
        ...
        kart.innerHTML = `
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
            <span style="font-size:12px;font-weight:700;color:${aktifMi ? '#4fc3f7' : 'var(--metin-soluk)'};">
              ${aktifMi ? '🎯 ' : ''}ID: ${soru.id}
            </span>
          ...
          <div style="font-size:14px;font-weight:600;color:#fff;margin-bottom:4px;">📝 ${soru.soru}</div>
          <div style="font-size:13px;color:#4fc3f7;">✅ ${soru.cevap}</div>
          ${soru.alternatifler && soru.alternatifler.length > 0
            ? `<div style="font-size:12px;color:var(--metin-soluk);margin-top:3px;">📎 ${soru.alternatifler.join(', ')}</div>`
            : ''}
        `;
      }
      ...
      window.adminSoruYukle = async function () {
        ...
        if (!hedefId || hedefId < 1) { toastGoster("Geçerli bir ID girin!"); return; }
        ...
        try {
          ...
          window._adminSoruData = { tumSorular: icerik, sha, harf, dosyaAdi, hedefId };
      ...
      window.adminSoruDuzenleAc = function (index) {
        ...
        if (!kart) return;
        kart.innerHTML = `
          <div style="font-size:12px;font-weight:700;color:var(--metin-soluk);margin-bottom:10px;">✏️ ID: ${soru.id} düzenleniyor</div>
          ...
          <textarea id="duzenle_soru_${index}" style="width:100%;background:#111827;border:1.5px solid rgba(255,255,255,0.12);
            ...
            outline:none;font-family:inherit;resize:vertical;min-height:70px;display:block;">${soru.soru}</textarea>
          ...
          <input type="text" id="duzenle_alt_${index}" autocomplete="off"
            ...
            value="${soru.alternatifler ? soru.alternatifler.join(', ') : ''}" />
      ...
      window.adminSoruKaydet = async function (index) {
        ...
        const yeniSoru = document.getElementById(`duzenle_soru_${index}`)?.value.trim();
        const yeniCevap = document.getElementById(`duzenle_cevap_${index}`)?.value.trim();
        const yeniAltRaw = document.getElementById(`duzenle_alt_${index}`)?.value.trim();

        if (!yeniSoru || !yeniCevap) { toastGoster("Soru ve cevap boş olamaz!"); return; }

        const yeniAlternatifler = yeniAltRaw
          ? yeniAltRaw.split(',').map(s => s.trim()).filter(Boolean)
          : [];
        ...
        try {
          const tokenSnap = await get(ref(veritabani, "config/githubToken"));
          const token = tokenSnap.val();
          if (!token) { toastGoster("❌ Token bulunamadı!"); return; }

          const { tumSorular, sha, dosyaAdi } = window._adminSoruData;
          tumSorular[index] = { ...tumSorular[index], soru: yeniSoru, cevap: yeniCevap, alternatifler: yeniAlternatifler };

          const apiUrl = `https://api.github.com/repos/berat-arsln/PassaWord/contents/questions/${dosyaAdi}_questions.json`;
          const yeniIcerikB64 = btoa(unescape(encodeURIComponent(JSON.stringify(tumSorular, null, 2))));
      ...
      window.adminSoruSil = async function (index) {
        ...
        if (silBtn) { silBtn.textContent = "⏳ Siliniyor..."; silBtn.disabled = true; }
        ...
        try {
          ...
          const apiUrl = `https://api.github.com/repos/berat-arsln/PassaWord/contents/questions/${dosyaAdi}_questions.json`;
          const yeniIcerikB64 = btoa(unescape(encodeURIComponent(JSON.stringify(yeniListe, null, 2))));
          ...
          if (silBtn) { silBtn.textContent = "🗑️ Bu Soruyu Sil"; silBtn.disabled = false; }
        }
      };
  ...
  if (!metin) { toastGoster('Metin girin!'); return; }
...
for (const blok of bloklar) {
    ...
    for (const satir of satirlar) {
      ...
      else if (satir.startsWith('a:')) a = satir.slice(2).split(',').map(x => x.trim()).filter(Boolean);
    }
    if (!s || !c) { sonuc.textContent = '❌ Hatalı format: ' + blok.slice(0, 30); return; }
  ...
  try {
    const tokenSnap = await get(ref(veritabani, 'config/githubToken'));
    const token = tokenSnap.val();
    if (!token) { sonuc.textContent = '❌ Token bulunamadı!'; return; }
    ...
    const eklenecekler = yeniSorular.map((s, i) => ({ id: maxId + i + 1, ...s }));
    ...
    sonuc.textContent = `✅ ${eklenecekler.length} soru eklendi! (ID: ${maxId + 1} - ${maxId + eklenecekler.length})`;
...
window.adminSorulariDuzelt = async function() {
  ...
  try {
    ...
    const yeniIcerikB64 = btoa(unescape(encodeURIComponent(JSON.stringify(duzeltilmis, null, 2))));
...
window.adminDuzenleSekmeGec = function(sekme) {
  document.getElementById('adminDuzenlePanel').style.display = sekme === 'duzenle' ? 'block' : 'none';
  document.getElementById('adminEklePanel').style.display = sekme === 'ekle' ? 'block' : 'none';
  document.getElementById('adminDuzenleSekme').style.background = sekme === 'duzenle' ? 'rgba(255,255,255,0.11)' : 'transparent';
  document.getElementById('adminDuzenleSekme').style.color = sekme === 'duzenle' ? '#fff' : 'var(--metin-soluk)';
  document.getElementById('adminEkleSekme').style.background = sekme === 'ekle' ? 'rgba(255,255,255,0.11)' : 'transparent';
